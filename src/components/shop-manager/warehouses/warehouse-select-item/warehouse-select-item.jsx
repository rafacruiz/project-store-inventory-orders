...
function WarehouseSelectItem({ warehouses, variant, search }) {
    ...
    return (
        <>
            {warehouses
            ?.filter((warehouse) => warehouse.name.toLowerCase().includes(search.toLowerCase()))
            .map((warehouse) => {
                ...
                if (variant === "list") {
                    ...
                    return (
                        <div key={warehouse.id} className="col-12 col-sm-6 col-md-4 col-lg-3">
                            <Link 
                                to={`/warehouse/${warehouse.id}`} 
                                className={`card h-100 text-decoration-none warehouse-card 
                                    ${isActive ? 'warehouse-card-active' : '' }`} 
                                >
